...
<body>
    ...
    <div class="container">
        <table class="table table-bordered table-sm">
            <!-- Include a table to display sales details -->
            <thead>
            ...
            <tbody>
                <?php
                // Connect to your database and fetch sales details
                ...
                if (isset($_GET['id'])) {
                    ...
                    try {
                        ...
                        // Fetch sales details based on the sales ID
                        $stmt = $pdo->prepare("SELECT pd.*, p.nama_produk, p.harga FROM penjualan_detail pd
                                              JOIN produk p ON pd.produk_id = p.produk_id
                                              WHERE pd.penjualan_id = :sales_id");
                    ...
                    echo "<tr><td colspan='5' class='text-center'>Sales ID not provided.</td></tr>";
                }
                ?>
            </tbody>
        </table>
        ...
        if (isset($_GET['id'])) {
            echo "<div class='text-right'>Total: {$total}</div>";
        }
        ?>
    </div>
